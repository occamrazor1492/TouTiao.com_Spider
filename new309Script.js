...
async function content(sessionCount) {
    ...
    var waitTime0 = Math.floor(Math.random() * 1000 + 2000)
    await sleep(waitTime0);
    console.log('get time finish');
  // get the newest time
    var item = 2
    var time = getElementByXpath('//*[@id="wrapper"]/div[2]/div[2]/div/div[3]/div/ul/li['+ item + ']/div/div[1]/div/div[2]/div[1]/span')
    if (time != null) {
      var timeText = time.innerText.match(/[0-9]{4}-[0-9]{2}-[0-9]{2}\s[0-9]{2}:[0-9]{2}/g)
      var newTime = new Date(timeText[0])
      var currentTime = new Date('2018-01-02 09:50')
    ...
    // get object from local storage
        ...
        for(i = 1; i < htmlPageLength; i+=1 ) {
            ...
            if (reading != null && comment != null && time != null && accountName != null) {
             ...
             var readingNumber = Number(readingText[0])
             var commentNumber = Number(commentText[0])

             if (commentNumber < 3) {
               var obj = {"reading": readingNumber, "comment": commentNumber, "time": timeText[0], "accountName": accountNameText, "link": hrefText }
             }
